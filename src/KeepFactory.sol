...
contract KeepFactory is Multicallable, Owned(tx.origin) {
    ...
    function deployKeep(
        ...
        assembly {
            let data := mload(0x40) // Get the current free memory pointer.
            
            // Store the salt (data) at the start of the memory segment.
            mstore(data, name)
            ...
            let extraLength := 0x22 // 0x20 (salt length) + 2

            let dataEnd := add(data, 0x20)
    ...
    function determineKeep(bytes32 name) public view virtual returns (address keep, bool deployed) {
        address implementation = address(keepTemplate);
